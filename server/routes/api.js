...
function endDateFromEndRules(endRules) {
    ...
    const hasDateParts =
        endRules.year && endRules.month && endRules.day;

    if (endRules.type === 'on' || (!endRules.type && hasDateParts)) {
...
function handleOccurrence(occDate) {
    // Stop if we've hit the after-count
    if (occurrencesGenerated >= maxOccurrences) return false;

    occurrencesGenerated += 1;

    const withinWindow =
        !isAfter(occDate, endWindow) &&
        !isBefore(occDate, startWindow);
    ...
    while (loopCount < MAX_LOOP) {
        ...
        for (const wd of weekdays) {
            ...
            const keepGoing = handleOccurrence(occDate);
            if (!keepGoing) break;
    ...
    // We'll iterate by stepping the anchor by interval units.
        ...
        while (loopCount < MAX_LOOP) {
            ...
            if (!isBefore(current, anchorStart)) {
                const keepGoing = handleOccurrence(current);
                if (!keepGoing) break;
            }
